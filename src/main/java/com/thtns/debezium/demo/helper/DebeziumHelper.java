package com.thtns.debezium.demo.helper;

import cn.hutool.core.bean.BeanUtil;
import cn.hutool.core.lang.Pair;
import cn.hutool.json.JSONUtil;

import com.thtns.debezium.demo.enums.EventTypeEnum;
import com.thtns.debezium.demo.enums.FilterJsonFieldEnum;
import com.thtns.debezium.demo.model.DebeziumListenerModel;
import com.thtns.debezium.demo.service.impl.ProcessDataContext;
...
public class DebeziumHelper {
	...
	private ProcessDataContext processDataContext;
	...
	public void handlePayload(List<RecordChangeEvent<SourceRecord>> recordChangeEvents,
		...
		for (RecordChangeEvent<SourceRecord> r : recordChangeEvents) {
			...
			DebeziumListenerModel changeListenerModel = getChangeDataInfo(sourceRecordChangeValue, changeMap);
			if (changeListenerModel == null) {
				continue;
			}
			//策略模式根据表名走不同的业务
			processDataContext.getDataStrategy(changeListenerModel);
	...
	private DebeziumListenerModel getChangeDataInfo(Struct sourceRecordChangeValue, Map<String, Object> changeMap) {
		// 操作类型过滤,只处理增删改
		...
		if (operation == Envelope.Operation.READ) {
			eventType = EventTypeEnum.READ.getType();
		...
		return BeanUtil.copyProperties(result, DebeziumListenerModel.class);
	}

	/**
	 * 过滤非重要变更数据
	 *
	 * @param sourceRecordChangeValue 数据
	...
	private Map<String, String> filterChangeData(Struct sourceRecordChangeValue, String currentTableName) {
		...
		//todo 根据表过滤字段
		resultMap.put(AFTER, JSONUtil.toJsonStr(afterMap));
		resultMap.put(BEFORE, JSONUtil.toJsonStr(beforeMap));
		return resultMap;
	}
	...
	//  *
	//  * @param currentTableName 当前表名
	//  * @param afterMap 变更后数据
	//  * @param beforeMap 变更前数据
	//  * @param filterColumnList 需要过滤的字段
	//  *
	//  * @return
	//  */
	...
	private Map<String, Object> getChangeTableInfo(Struct sourceRecordChangeValue) {
		...
		Map<String, Object> map = struct.schema()
				.fields()
				.stream()
				.map(Field::name)
	...
	@Autowired
	public void setProcessDataContext(ProcessDataContext processDataContext) {
		this.processDataContext = processDataContext;
	}
}
